...
int dbd_db_login(SV* dbh, imp_dbh_t* imp_dbh, char* dbname, char* user,
	...
	*host++ = '\0';
    ...
    if (!dbd_db_connect(imp_dbh->svsock, host, user, password, dbname)) {
	DO_ERROR(dbh, JW_ERR_CONNECT, imp_dbh->svsock);
	if (copy) free(copy);
	return FALSE;
    }

#if !defined(DBD_MYSQL)  ||  !defined(MYSQL_VERSION_ID)  ||  (MYSQL_VERSION_ID < 32200)
    /*
