...
int MyConnect(dbh_t *sock, char* host, char* port, char* user,
	      char* password, char* dbname, imp_dbh_t *imp_dbh) {
    int portNr;
   ...
   if (!initialized) {
    ...
    {
	mysql_init(*sock);
	if (imp_dbh  &&  imp_dbh->compression) {
...
static int _MyLogin(imp_dbh_t *imp_dbh) {
	...
	*svp  &&  SvTRUE(*svp)) {
        imp_dbh->compression = 1;
    } else {
        imp_dbh->compression = 0;
    }
#endif
    ...
    return MyConnect(&imp_dbh->svsock, host, port, user, password, dbname,
		     imp_dbh);
}
...
SV* dbd_db_FETCH_attrib(SV* dbh, imp_dbh_t* imp_dbh, SV* keysv) {
	...
	if (strEQ(key, "insertid")) {
	    return sv_2mortal(newSViv((IV) mysql_insert_id(imp_dbh->svsock)));
	}
	break;
#endif
      case 'p':
...
int dbd_st_execute(SV* sth, imp_sth_t* imp_sth) {
	...
	if (!imp_sth->cda) {
#if defined(DBD_MYSQL)
	    imp_sth->insertid = (int) mysql_insert_id(imp_dbh->svsock);
#endif
	} else {
