...
sub Initialize ($) {
	...
	($libdir) = $self->SearchLibs
	    (\@gooddirs, \@mysqldirs,
	     ["lib/mysql/libmysqlclient.a", "lib/mysql/libmysqlclient.so",
    ...
    $extralibs .= $self->CheckForLibGcc($libdir);
    ...
    my $defs =
	"-DDBD_MYSQL -DMyConnect=mysql_dr_connect -DMyLogin=mysql_dr_login";
