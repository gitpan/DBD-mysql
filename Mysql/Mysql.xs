   ...
   CODE:
{
  ...
  case 'I':
    ...
    } else if (strEQ(key, "ISNUM")) {
	MYPERL_FETCH_INTERNAL(av_push(av,(SV*)newSViv((IV)IS_NUM(curField->type))););
#ifdef DBD_MYSQL
...
#ifdef DBD_MYSQL
	MYPERL_FETCH_INTERNAL(av_push(av,(SV*)newSViv((IV)curField->max_length)););
#else
	MYPERL_FETCH_INTERNAL(av_push(av,(SV*)newSViv((IV)curField->length)););
#endif
    }
...
unixtimetodate(package = Package, clock)
   ...
   CODE:
     ...
     RETVAL = ""; /* Suppress warnings for uninitialized RETVAL */
#endif
   OUTPUT:
     RETVAL
...
#else
     croak("not implemented");
     RETVAL = 0; /* Suppress warnings for uninitialized RETVAL */
#endif
   OUTPUT:
     RETVAL
...
reloadacls(handle)
     ...
     CODE:
     {
      ...
      readMYSOCKET;
      if (validSOCKET) {
#if defined(DBD_MYSQL)  &&  defined(REFRESH_GRANT)
	checkRETVAL(mysql_refresh(sock,REFRESH_GRANT | REFRESH_LOG | REFRESH_TABLES));
#else
      ...
      the hash table in the key DATABASE. */
{
  ...
  result = MyConnect(&sock, host, user, password, NULL);
...
/* A successful query returns a statement handle in the
  ...
  if (tmp < 0 ) {
	...
	sv_setpv( ST(0), "0e0");
      ...
      hv_store(hv, "INSERTID", 8, newSViv((IV)mysql_insert_id(sock)), 0);
      hv_store(hv,"SOCK",9,newSViv((IV)sock),0);
      rv = newRV((SV*)hv);
      stash = gv_stashpv((char*) StPackage, TRUE);
      ST(0) = sv_2mortal(sv_bless(rv, stash));      
